      ...
      $.fn.jcaption = function(settings) {
        settings = $.extend(
          {
            ...
            copyAlignmentToClass:
              Drupal.settings.jcaption.jcaption_copyAlignmentToClass,
            copyFloatToClass:
              Drupal.settings.jcaption.jcaption_copyFloatToClass,
            copyClassToClass:
              Drupal.settings.jcaption.jcaption_copyClassToClass,
        ...
        return $(this).each(function() {
          ...
          $(this).bind("load", function() {
            ...
            if (
              cleanimage.attr(settings.imageAttr) != undefined &&
              (cleanimage.attr(settings.imageAttr).length > 0 ||
                !settings.requireText)
            ) {
              //Wrap the image with the caption span
              image.wrap(
                "<" +
                  settings.wrapperElement +
                  " class='" +
              ...
              var span = image
                ...
                .append(
                  "<" +
                    settings.captionElement +
                    ">" +
                    cleanimage.attr(settings.imageAttr) +
                    "</" +
                    settings.captionElement +
                    ">"
                );
                ...
                $(this)
                  ...
                  .hover(
                    function() {
                      $(this)
                        .find("p")
                        .animate(settings.show, settings.showDuration);
                    },
                    function() {
                      $(this)
                        .find("p")
                        .animate(settings.hide, settings.hideDuration);
                    }
                  );
              }
  ...
  Drupal.behaviors.contextualLinks = {
    attach: function(context) {
      ...
      $(".contextual-links-wrapper")
        .parent()
        .addClass("contextual-links-region clearfix");
